#[
    scene setup for 11.2 in the Ray Tracing in One Weekend
]#
...
proc render_11_2*() =
    ...
    var
        lookFrom: Point3 = newPoint3(3, 3, 2)
        lookAt: Point3 = newPoint3(0, 0, -1)
        vup: Vec3 = newVec3(0, 1, 0)
        distToFocus: float64 = (lookFrom-lookAt).length()
        aperture: float64 = 2.0
        ...
        materialCenter: Lambertian = newLambertian(newColor(0.1, 0.2, 0.5))
        materialLeft: Dielectric = newDielectric(1.5)
        materialRight: Metal = newMetal(newColor(0.8, 0.6, 0.2), 0.0)
    ...
    world.add(newSphere(newPoint3(-1.0,    0.0, -1.0),  -0.45, materialLeft))
    world.add(newSphere(newPoint3( 1.0,    0.0, -1.0),   0.5, materialRight))

    var outputFile : File = open("./outputs/scene_11_2.ppm", FileMode.fmWrite)
